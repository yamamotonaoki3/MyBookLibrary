...
export default async function UserProfilePage({ params }: Props) {
  ...
  const canViewDetails = isSelf || isMutual;

  const [favoriteAuthors, readingStatuses] = canViewDetails
    ? await Promise.all([
  ...
  return (
    <main className="flex flex-col px-4 py-6 lg:flex-1 lg:overflow-hidden lg:px-8 lg:py-8">
      <div className="flex-1 overflow-y-auto">
        ...
        {!canViewDetails ? (
          <p className="rounded-lg border border-zinc-200 bg-zinc-50 p-6 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
            相互フォローになると、{user.name}
            さんのお気に入り著者や読んでいる本が見られるようになります。
          </p>
        ) : (
