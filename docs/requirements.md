...
受賞登録（賞タブ＋年度フィルター付き一覧）・CSV インポート・手動書籍登録／マージ・受賞データ CSV エクスポート・通報レビュー管理・ユーザー管理・お問い合わせ管理・監査ログ閲覧・統計確認など、管理者専用の操作機能（`admin` ロールのみ）。管理者パネルは「設定・管理・監査ログ」の3タブ構成。
...
| パス | 画面名 | 説明 |
| --- | --- | --- |
| `/admin` | 管理者パネル | 「設定」「管理」「監査ログ」の3タブ構成：書籍登録（楽天/NDL）・手動書籍登録／マージ・近隣図書館の登録・受賞管理・CSV インポート／エクスポート・通報レビュー削除・ユーザー管理・お問い合わせ管理・統計・監査ログ閲覧 |
