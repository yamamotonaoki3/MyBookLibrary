...
新刊通知・いいね通知・フォロー関連通知（フォロー・相互フォロー成立後の活動通知）をアプリ内で受け取り、既読管理できる機能。PWA インストール時はアイコンバッジにも反映される。
...
ユーザー同士がフォロー・フォロー解除でき、フォロー中ユーザーの一覧を確認できる機能。同じ著者をお気に入り登録している他ユーザーを、共通著者数の多い順におすすめフォロー候補として提示する。他ユーザーのお気に入り著者・読んでいる本は相互フォローの有無に関わらず閲覧でき、相互フォロー成立時のみ相手の活動が通知される。
