...
| 項目 | 内容 |
| --- | --- |
| なぜ困難か | Edge runtime 前提で、Jest の Node 環境では実体を動かす意味がない |
| どうテストするか | **`proxy.ts` 自体はテストしない。** 実質のテスト対象は `src/auth.config.ts` の `authorized` コールバック。これを直接呼ぶ単体テストで、公開パス／保護パス／管理者パス／リダイレクト先を網羅する。`config.matcher` の正規表現は「パス文字列 → マッチするか」のテーブルテストで別途検証できる |
| 状態 | 未対応（Phase 3 で実施） |
...
1. **APIキー（`RAKUTEN_APP_ID` / `CALIL_API_KEY`）をダミー値にしたまま全テストが通ること** — 実キーが不要＝実APIを叩いていない、が最も簡潔な証明。CIに実キーを置かない運用も同時に成立する
2. fetch ガードが効いていることを、モックを意図的に外したテストが**失敗すること**で確認する
3. E2E 実行中にネットワークを切っても通ること
